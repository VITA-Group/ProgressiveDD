...
def main(args, images_best, labels_best):
    ...
    if args.interval == 1 or args.save_dir is None:
        experiment_name = f'{args.dataset}_{args.model}_{args.eval_mode}_ipc{args.ipc}_max{args.max_start_epoch}_syn{args.syn_steps}_real{args.expert_epochs}_img{args.lr_img}_{args.lr_lr}_{args.lr_teacher}_increase'
    ...
    if not os.path.exists(args.save_dir):
        ...
        while os.path.exists(os.path.join(args.save_dir, "replay_buffer_{}.pt".format(n))):
            ...
            n += 1
        if n > 0:
            images_best = torch.load(os.path.join(args.save_dir, 'images_best.pt'))
            labels_best = torch.load(os.path.join(args.save_dir, 'labels_best.pt'))
            return args, images_best, labels_best
        else:
    ...
    if args.interval > 1:
        images_best = images_best.to(image_syn.device)
        labels_best = labels_best.to(label_syn.device)
        
        image_syn = torch.cat([images_best, image_syn], 0)
        label_syn = torch.cat([labels_best, label_syn], 0)
        masked_num = images_best.shape[0]
    else:
        masked_num = 0
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool:
            for model_eval in model_eval_pool:
                ...
                if model_eval == args.model and it == eval_it_pool[-1] and args.num_intervals > 0 and args.interval < args.num_intervals:
                    ...
                    images_best = image_syn
                    labels_best = label_syn
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                if save_this_it:
                    ...
                    images_best = image_save.cpu().clone()
                    labels_best = label_syn.cpu().clone()
        ...
        grand_loss.backward()
        image_syn.grad.data[:masked_num] = 0
        grad_norm = image_syn.grad[masked_num:].reshape(image_syn.shape[0] - masked_num, -1)
        grad_norm = torch.norm(grad_norm, dim=-1)
        logs = {"Grand_Loss": grand_loss.detach().cpu(),
                   "Start_Epoch": start_epoch}
        for i in range(grad_norm.shape[0]):
            logs[f'grad_norm_{i}'] = grad_norm[i]
...
if __name__ == '__main__':
    ...
    images_best, labels_best = None, None

    if args.buffer_path != "./buffers":
        images_best = torch.load(args.buffer_path + "/images_best.pt")
    ...
    for interval in range(args.start_interval, args.num_intervals+1):
        ...
        if args.use_more_at_first_interval and interval > args.start_interval:
            args.ipc = original_ipc
        ...
        args, images_best, labels_best = main(args, images_best, labels_best)

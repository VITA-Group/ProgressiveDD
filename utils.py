...
from networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN, ResNet18BN_AP, ResNet18_AP, ResNet10, ResNet10BN 
# import model_densenet as DN
class Config:
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None, return_testset=False):
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=True):
    ...
    if model == 'MLP':
        ...
        net = ResNet10BN(channel=channel, num_classes=num_classes)
        ...
        net = DN.densenet_cifar(num_classes)
    elif model == 'ResNet10BN_IDC':
        net = RN.ResNet('cifar10', 10, num_classes, 'batch', -1, nch=3)
    else:
        net = None
...
def warmup_lr(epoch, optimizer, one_epoch_step, warmup, lr):

    overall_steps = warmup
    current_steps = epoch*one_epoch_step 

    lr_ = lr * current_steps/overall_steps
    new_lr = min(lr_, lr)
    print(new_lr)
    for p in optimizer.param_groups:
        p['lr'] = new_lr

def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, return_loss=False, texture=False, save_model = False, save_model_prefix = None, stage=1, warmup=0, return_optimizer=False, init_state=None):
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        ...
        if ep <= warmup and warmup > 0:
            warmup_lr(ep, optimizer, 1, warmup, lr)
        acc_train_list.append(acc_train)
        loss_train_list.append(loss_train)
        if ep == Epoch:
            with torch.no_grad():
                loss_test, acc_test = epoch('test', testloader, net, optimizer, criterion, args, aug=False)
        if ep % 5 == 0 and save_model:
    ...
    if return_loss:
        ...
        return net, acc_train_list, acc_test, optimizer
...
def check_sparsity(ws):
    total = 0
    zeroes = 0
    for w in ws:
        zeroes += torch.sum(w == 0)
        total += w.numel()
    print(zeroes / total)
    return zeroes/total